...
package cn.vip.dw.bluetoothprinterlib.zxing.oned.rss;
...
import cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair;
...
public final class RSS14Reader extends AbstractRSSReader {
  ...
  private final List<cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair> possibleLeftPairs;
  private final List<cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair> possibleRightPairs;
  ...
  public Result decodeRow(int rowNumber,
    ...
    cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair leftPair = decodePair(row, false, rowNumber, hints);
    addOrTally(possibleLeftPairs, leftPair);
    row.reverse();
    cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair rightPair = decodePair(row, true, rowNumber, hints);
    addOrTally(possibleRightPairs, rightPair);
    row.reverse();
    int lefSize = possibleLeftPairs.size();
    for (int i = 0; i < lefSize; i++) {
      cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair left = possibleLeftPairs.get(i);
      if (left.getCount() > 1) {
        int rightSize = possibleRightPairs.size();
        for (int j = 0; j < rightSize; j++) {
          cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair right = possibleRightPairs.get(j);
          if (right.getCount() > 1) {
  ...
  private static void addOrTally(Collection<cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair> possiblePairs, cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair pair) {
    if (pair == null) {
      return;
    }
    boolean found = false;
    for (cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair other : possiblePairs) {
  ...
  private static Result constructResult(cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair leftPair, cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair rightPair) {
  ...
  private static boolean checkChecksum(cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair leftPair, cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair rightPair) {
  ...
  private cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair decodePair(BitArray row, boolean right, int rowNumber, Map<DecodeHintType,?> hints) {
    try {
      ...
      return new cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.Pair(1597 * outside.getValue() + inside.getValue(),
  ...
  private DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern, boolean outsideChar)
    ...
    if (outsideChar) {
      ...
      int vOdd = cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.RSSUtils.getRSSvalue(oddCounts, oddWidest, false);
      int vEven = cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.RSSUtils.getRSSvalue(evenCounts, evenWidest, true);
      ...
      int evenWidest = 9 - oddWidest;
      int vOdd = cn.vip.dw.bluetoothprinterlib.zxing.oned.rss.RSSUtils.getRSSvalue(oddCounts, oddWidest, true);
